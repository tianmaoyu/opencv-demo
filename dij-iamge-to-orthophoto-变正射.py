...
def pixelcoord_to_geocoord(image_file: str, pixel: float, pixel_x: int, pixel_y: int) -> dict:
    ...
    return object_point_in_geo[0], object_point_in_geo[1]
...
def geocoord_to_pixelcoord(image_file: str, pixel: float, target_longitude: float, target_latitude: float) -> dict:
    ...
    move_vector = np.array([12624065.1656054,2532806.24386257, altitude])
...
for pixel_x, pixel_y in source_pixel_list:
    ...
    source_geocoord_list.append([latitude, longitude])

source_geocoord_list = np.array(source_geocoord_list)
...
print(source_geocoord_list.tolist())
print(source_pixel_list.tolist())

#目标像素坐标点
target_pixel_list = []
for longitude,latitude in source_geocoord_list:
    pixel_x, pixel_y = geocoord_to_pixelcoord(image_file, pixel, longitude, latitude)
    target_pixel_list.append([round(pixel_x),round(pixel_y)])

target_pixel_list=np.array(target_pixel_list)
print(target_pixel_list.tolist())
