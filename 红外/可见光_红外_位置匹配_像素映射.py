...
# 4个点对应的4个地理坐标
source_geocoord_list=pixelcoord_to_geocoord(t_image, t_pixel,source_pixel_list)
print("红外-地理坐标点",source_geocoord_list.tolist())

#4个地理坐标- 在 新的视觉角下的 像素坐标- 几何中心正上方，垂直向下 视觉拍摄
target_pixel_list = geocoord_to_pixelcoord(W_image, w_pixel, source_geocoord_list)
print("广角-目标像素点:",target_pixel_list.tolist())


# 读取图像 B
image_w = cv2.imread(W_image)
# 将这些点转换为整数，因为像素位置只能是整数
target_pixel_list = target_pixel_list.astype(int)

# 连接四个点，形成四边形（闭合路径）
cv2.polylines(image_w, [target_pixel_list], isClosed=True, color=(255, 0, 0), thickness=10)
target_pixel_list[0:2,1]+=200
target_pixel_list[2:4,1]-=200
cv2.polylines(image_w, [target_pixel_list], isClosed=True, color=(0, 255, 0), thickness=10)
# 显示图像 B，其中红色线表示四个角点组成的四边形
plt.figure(figsize=(28, 10))

plt.subplot(1,2,1)
plt.imshow(cv2.cvtColor( cv2.imread(t_image), cv2.COLOR_BGR2RGB))
# plt.title('T')
plt.axis('off')

plt.subplot(1,2,2)
plt.imshow(cv2.cvtColor(image_w, cv2.COLOR_BGR2RGB))
# plt.title('W')
plt.axis('off')
