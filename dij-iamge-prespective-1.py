...
def geocoord_to_pixelcoord(image_file: str, pixel: float, target_longitude: float, target_latitude: float) -> dict:
    ...
    pixel_y = target_point[2] + height / 2
    return pixel_x, pixel_y


def image_to_geo(image_file: str,pixel:float):
    """
    把照片坐标 转到 地面坐标
    :param image_file:
    :return:
    ...
    camera1_vector = np.array([x, y, altitude])
    ...
    camera1_rotate = Rz1 @ Ry1 @ Rx1

    A1 = vector1 @ camera1_rotate
    # A1 =  camera1_rotate

    longitude1, latitude1 = pixelcoord_to_geocoord(image_file, pixel, 0, 0)
    longitude2, latitude2 = pixelcoord_to_geocoord(image_file, pixel, 4000, 3000)
    longitude3, latitude3 = pixelcoord_to_geocoord(image_file, pixel, 4000, 0)
    longitude4, latitude4 = pixelcoord_to_geocoord(image_file, pixel, 0, 3000)
    x1, y1 = wgs84_to_web(latitude1,longitude1 )
    x2, y2 = wgs84_to_web(latitude2, longitude2)
    x3, y3 = wgs84_to_web(latitude3, longitude3)
    x4, y4 = wgs84_to_web(latitude4, longitude4)
    x0=(x1+x2+x3+x4)/4
    y0 = (y1 + y2 + y3 + y4) / 4
    camera2_vector = np.array([x0, y0, 0])
    vector2 = np.array([
        [1, 0, 0, x0],
        [0, 1, 0, y0],
        [0, 0, 1, 0],
    ...
    pitch2 = math.radians(pitch2)
    Ry2 = np.array([
        [np.cos(pitch2), 0, np.sin(pitch2),0],
        [0, 1, 0,0],
        [-np.sin(pitch2), 0, np.cos(pitch2),0],
        [0, 0, 0, 1],
    ])

    camera2_rotate = Ry2
    A2= vector2@A1@Ry2@np.linalg.inv(A1)
    # A2 = vector2 @ A1 @ Ry2
    return A2

def image_to_camera_test_v1():
    pixel = 1.6 / 1000_000
    points=[]
    for x in range(4000):
        for y in range(3000):
...
#循环1200 万个点计算
def camera_to_geo_test_v1(points:np.ndarray):
    for point in points:
        pass
...
# 相对变换矩阵
relitive_matrix=image_to_geo(image_file,pixel)
print(relitive_matrix)

# points=image_to_camera_test_v2()
# print(points)
#
# points=image_to_camera_test_v1()
# print(points)


# test=np.array([ 0.0044, -0.0032, -0.0024])/pixel
